...
export default function ForgotPasswordPage() {
  ...
  return <AuthShell title="Reset your password" description="We will send a secure reset link to your work email.">{sent?<div className="password-sent"><CheckCircle2/><b>Check your inbox</b><p>If an account exists for that address, a reset link is on its way.</p></div>:<form className="auth-form" onSubmit={submit}><label>Email address<input name="email" type="email" autoComplete="email" placeholder="you@company.com" required/></label>{error&&<p className="form-error">{error}</p>}<button className="button button--primary button--large button--full" disabled={loading}>{loading?<><LoaderCircle className="spin"/> Sending...</>:'Send reset link'}</button></form>}<p className="auth-switch"><Link href="/login"><ArrowLeft size={14}/> Return to sign in</Link></p></AuthShell>;
}

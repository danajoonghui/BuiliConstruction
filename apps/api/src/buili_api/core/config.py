...
class Settings(BaseSettings):
    ...
    worker_database_url: str | None = Field(
        default=None, validation_alias="BUILI_WORKER_DATABASE_URL"
    )
    auto_create_schema: bool = Field(default=True, validation_alias="BUILI_AUTO_CREATE_SCHEMA")
    public_api_url: str = Field(
        default="http://localhost:8000", validation_alias="BUILI_PUBLIC_API_URL"
    )
    ...
    jwt_secret: SecretStr = Field(
        default=SecretStr("local-dev-secret-change-me-32chars"), validation_alias="BUILI_JWT_SECRET"
    )
    jwt_issuer: str = Field(
        default="https://api.builiconstruction.com", validation_alias="BUILI_JWT_ISSUER"
    )
    ...
    cookie_samesite: Literal["lax", "strict", "none"] = Field(
        default="lax", validation_alias="BUILI_COOKIE_SAMESITE"
    )
    origin_verify_secret: SecretStr | None = Field(
        default=None, validation_alias="BUILI_ORIGIN_VERIFY_SECRET"
    )
    frontend_url: str = Field(
        default="http://localhost:3000", validation_alias="BUILI_FRONTEND_URL"
    )
    email_backend: Literal["disabled", "log", "ses"] = Field(
        default="disabled", validation_alias="BUILI_EMAIL_BACKEND"
    )
    require_email_verification: bool = Field(
        default=False, validation_alias="BUILI_REQUIRE_EMAIL_VERIFICATION"
    )
    auth_token_minutes: int = Field(default=30, validation_alias="BUILI_AUTH_TOKEN_MINUTES")
    auth_rate_limit_per_minute: int = Field(
        default=12, validation_alias="BUILI_AUTH_RATE_LIMIT_PER_MINUTE"
    )

    oidc_issuer: str = Field(
        default="https://accounts.google.com", validation_alias="BUILI_OIDC_ISSUER"
    )
    oidc_client_id: str | None = Field(default=None, validation_alias="BUILI_OIDC_CLIENT_ID")

    storage_backend: Literal["local", "s3"] = Field(
        default="local", validation_alias="BUILI_STORAGE_BACKEND"
    )
    ...
    max_upload_bytes: int = Field(
        default=95 * 1024 * 1024, validation_alias="BUILI_MAX_UPLOAD_BYTES"
    )
    ...
    max_archive_compression_ratio: int = Field(
        default=100, validation_alias="BUILI_MAX_ARCHIVE_COMPRESSION_RATIO"
    )
    max_pdf_pages: int = Field(default=500, validation_alias="BUILI_MAX_PDF_PAGES")
    max_extracted_text_chars: int = Field(
        default=5_000_000, validation_alias="BUILI_MAX_EXTRACTED_TEXT_CHARS"
    )
    max_image_pixels: int = Field(default=100_000_000, validation_alias="BUILI_MAX_IMAGE_PIXELS")

    job_backend: Literal["local", "sqs"] = Field(
        default="local", validation_alias="BUILI_JOB_BACKEND"
    )
    ...
    job_visibility_timeout_seconds: int = Field(
        default=900, validation_alias="BUILI_JOB_VISIBILITY_TIMEOUT_SECONDS"
    )
    ...
    openai_transcribe_model: str = Field(
        default="gpt-4o-transcribe", validation_alias="OPENAI_TRANSCRIBE_MODEL"
    )
    ...
    openai_timeout_seconds: float = Field(
        default=30.0, validation_alias="BUILI_OPENAI_TIMEOUT_SECONDS"
    )
    ai_max_image_bytes: int = 20 * 1024 * 1024
    ai_max_audio_bytes: int = 25 * 1024 * 1024
    ai_max_context_characters: int = 120_000
    spatial_processor_url: str | None = Field(
        default=None, validation_alias="BUILI_SPATIAL_PROCESSOR_URL"
    )

    tripo_enabled: bool = Field(default=False, validation_alias="BUILI_TRIPO_ENABLED")
    tripo_api_key: SecretStr | None = Field(default=None, validation_alias="TRIPO_API_KEY")
    tripo_model_version: str = Field(
        default="P1-20260311", validation_alias="BUILI_TRIPO_MODEL_VERSION"
    )
    tripo_api_base_url: str = Field(
        default="https://api.tripo3d.ai/v2/openapi",
        validation_alias="BUILI_TRIPO_API_BASE_URL",
    )
    tripo_poll_interval_seconds: float = Field(
        default=2.0, validation_alias="BUILI_TRIPO_POLL_INTERVAL_SECONDS"
    )
    tripo_task_timeout_seconds: int = Field(
        default=420, validation_alias="BUILI_TRIPO_TASK_TIMEOUT_SECONDS"
    )
    tripo_max_asset_bytes: int = Field(
        default=80 * 1024 * 1024, validation_alias="BUILI_TRIPO_MAX_ASSET_BYTES"
    )
    tripo_download_host_suffixes: str = Field(
        default="tripo3d.ai,tripo3d.com",
        validation_alias="BUILI_TRIPO_DOWNLOAD_HOST_SUFFIXES",
    )

    demo_mode: bool = Field(default=False, validation_alias="BUILI_DEMO_MODE")
    demo_email: str = Field(
        default="jordan@demo.builiconstruction.com", validation_alias="BUILI_DEMO_EMAIL"
    )
    demo_password: SecretStr = Field(
        default=SecretStr("ChangeMe-Demo-2026!"), validation_alias="BUILI_DEMO_PASSWORD"
    )
    demo_evidence_path: Path = Field(
        default=Path("../../buili_demo_evidence"), validation_alias="BUILI_DEMO_EVIDENCE_PATH"
    )
    ...
    @property
    def tripo_download_hosts(self) -> tuple[str, ...]:
        return tuple(
            item.strip().lower().lstrip(".")
            for item in self.tripo_download_host_suffixes.split(",")
            if item.strip()
        )
    ...
    def validate_secure_runtime(self) -> "Settings":
        if self.environment == "production":
            ...
            if len(jwt_secret) < 32 or any(
                ...
                raise ValueError(
                    "BUILI_JWT_SECRET must be an independent secret of at least 32 characters"
                )
            if self.demo_mode:
                raise ValueError("BUILI_DEMO_MODE is forbidden in production")
            if self.auto_create_schema:
                raise ValueError(
                    "Run Alembic migrations and disable AUTO_CREATE_SCHEMA in production"
                )
            ...
            if self.cookie_domain:
                raise ValueError(
                    "Production auth cookies must be host-only; do not set BUILI_COOKIE_DOMAIN"
                )
        ...
        if self.malware_scanner_backend == "test" and self.environment != "test":
            raise ValueError(
                "The deterministic test scanner is available only in test environments"
            )
        ...
        if self.external_ai_enabled:
            ...
            if not self.openai_model.strip():
                raise ValueError(
                    "OPENAI_MODEL must be explicitly pinned when external AI is enabled"
                )
            if not self.openai_transcribe_model.strip() or not self.openai_embedding_model.strip():
                raise ValueError(
                    "OpenAI transcription and embedding model IDs must be explicitly pinned"
                )
        if self.tripo_enabled:
            if not self.tripo_api_key or not self.tripo_api_key.get_secret_value().strip():
                raise ValueError("TRIPO_API_KEY is required when BUILI_TRIPO_ENABLED=true")
            if not self.tripo_model_version.strip():
                raise ValueError("BUILI_TRIPO_MODEL_VERSION must be explicitly pinned")
            if not self.tripo_api_base_url.startswith("https://"):
                raise ValueError("Tripo API base URL must use HTTPS")
            if not self.tripo_download_hosts:
                raise ValueError("At least one trusted Tripo download host suffix is required")
        if not 0.5 <= self.tripo_poll_interval_seconds <= 30:
            raise ValueError("Tripo poll interval must be between 0.5 and 30 seconds")
        if not 30 <= self.tripo_task_timeout_seconds <= 1800:
            raise ValueError("Tripo task timeout must be between 30 and 1800 seconds")
        if not 5 <= self.job_heartbeat_seconds < self.job_visibility_timeout_seconds:
            raise ValueError(
                "Job heartbeat must be at least 5 seconds and lower than the visibility timeout"
            )
        return self

...
class DocumentRevision(Base, TimestampMixin):
    ...
    upload_id: Mapped[str | None] = mapped_column(ForeignKey("uploads.id", ondelete="SET NULL"), nullable=True)
...
class Issue(Base, TimestampMixin):
    ...
    recommended_action: Mapped[str] = mapped_column(String(64), default="additional_evidence_required")
...
class IssueSource(Base):
    ...
    revision_id: Mapped[str] = mapped_column(ForeignKey("document_revisions.id", ondelete="CASCADE"), index=True)
...
class Report(Base, TimestampMixin):
    ...
    __table_args__ = (UniqueConstraint("issue_id", "kind", "version", name="uq_report_issue_kind_version"),)
...
class ReportArtifact(Base):
    ...
    id: Mapped[str] = mapped_column(
        String(40), primary_key=True, default=lambda: new_id("rat")
    )
...
class SpatialScene(Base, TimestampMixin):
    ...
    organization_id: Mapped[str] = mapped_column(ForeignKey("organizations.id", ondelete="CASCADE"), index=True)
    project_id: Mapped[str] = mapped_column(ForeignKey("projects.id", ondelete="CASCADE"), index=True)
    source_revision_id: Mapped[str] = mapped_column(ForeignKey("document_revisions.id", ondelete="RESTRICT"), index=True)
    plan_graph_id: Mapped[str] = mapped_column(ForeignKey("plan_graphs.id", ondelete="RESTRICT"), index=True)
    version: Mapped[int] = mapped_column(Integer, default=1)
    ...
    created_by: Mapped[str] = mapped_column(ForeignKey("users.id", ondelete="RESTRICT"))
    __table_args__ = (
        UniqueConstraint("project_id", "source_revision_id", "version", name="uq_spatial_scene_version"),
        Index(
...
class PlanGraph(Base, TimestampMixin):
    ...
    created_by: Mapped[str] = mapped_column(ForeignKey("users.id", ondelete="RESTRICT"))
    __table_args__ = (
        UniqueConstraint("project_id", "source_revision_id", "version", name="uq_plan_graph_version"),

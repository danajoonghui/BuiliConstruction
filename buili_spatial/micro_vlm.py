...
from .model_io import safe_torch_checkpoint_load
from .semantic_scene import SemanticObject, SemanticOpening, SemanticRoomLabel
        ...
        def __init__(
            ...
            *,
            ...
            self.pos = nn.Parameter(
                torch.zeros(1, tokens_per_side * tokens_per_side + 1, dim)
            )
            ...
            self.box_head = nn.Sequential(
                nn.Linear(dim, dim), nn.GELU(), nn.Linear(dim, 4)
            )

        def forward(
            self, images: torch.Tensor, task_ids: torch.Tensor
...
def _load_model(path: Path) -> tuple[Any | None, dict[str, Any]]:
    ...
    try:
        checkpoint = safe_torch_checkpoint_load(
            torch,
            path,
            expected_sha256_env="BUILI_PLAN2FIELD_MICRO_VLM_SHA256",
        )
    ...
    try:
        ...
        return None, {
            "enabled": False,
            "reason": "model_artifact_incompatible",
            "error": str(exc),
            "path": str(path),
        }
    device = (
...
def _patch_boxes(
    width: int, height: int, *, patch: int, stride: int
...
def _model_predictions(
    ...
    *,
    ...
    for batch_start in range(0, len(boxes), batch_size):
        ...
        task_ids = torch.full(
            (len(batch_boxes),), task_id, dtype=torch.long, device=device
        )
        with torch.no_grad():
...
def _nms(
    predictions: list[MicroVLMPrediction], iou_threshold: float = 0.25
) -> list[MicroVLMPrediction]:
    ...
    for pred in sorted(
        predictions,
        key=lambda item: (-item.confidence, item.label, item.bbox_px, item.source),
    ):
        if any(
            pred.label == item.label and iou(pred, item) > iou_threshold
            for item in kept
        ):
            continue
        kept.append(pred)
...
def _nearest_room_distance(
    ...
    cx, cy = center
    return min(
        math.hypot(cx - label.center_px[0], cy - label.center_px[1]) for label in labels
    )


def _prediction_to_object(
    pred: MicroVLMPrediction, index: int
) -> SemanticObject | None:
...
def _prediction_to_opening(
    pred: MicroVLMPrediction, index: int
) -> SemanticOpening | None:
...
def detect_micro_vlm_plan_elements(
    ...
    *,
    ...
    if os.environ.get("BUILI_PLAN2FIELD_MICRO_VLM_DISABLE", "").lower() in {
        "1",
        "true",
        "yes",
    }:
    ...
    verify_threshold = float(
        os.environ.get("BUILI_PLAN2FIELD_MICRO_VLM_VERIFY_CONF", "0.55")
    )
    correction_threshold = float(
        os.environ.get("BUILI_PLAN2FIELD_MICRO_VLM_CORRECT_CONF", "0.82")
    )
    ...
    boxes = _patch_boxes(image.width, image.height, patch=patch, stride=stride)[
        :max_patches
    ]
    deadline = start + max_seconds
    batch_size = int(os.environ.get("BUILI_PLAN2FIELD_MICRO_VLM_BATCH", "16"))
    accepted_objects, accepted_openings, proposal_metadata = (
        _verify_existing_candidates(
            model,
            image,
            classes=classes,
            image_size=image_size,
    ...
    for batch_start in range(0, len(boxes), batch_size):
        ...
        predictions.extend(
            pred for pred in batch_predictions if pred.confidence >= threshold
        )
    ...
    for pred in _nms(predictions):
        if _near_existing(
            pred,
            [*existing_objects, *accepted_objects],
            [*existing_openings, *accepted_openings],
        ):
            rejected += 1
            continue

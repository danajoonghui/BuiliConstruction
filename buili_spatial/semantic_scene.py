...
def _ellipse_points(
    ...
    *,
    ...
    for index in range(segments):
        ...
        points.append(
            (cx + ux * local_x + vx * local_z, cz + uz * local_x + vz * local_z)
        )
    return points
...
def _add_asset_box(
    ...
    *,
    ...
    _add_prism(
        renderer,
        _rect_points(
            center, width, depth, angle_deg, offset_x=offset_x, offset_z=offset_z
        ),
        y0=y0,
        y1=y1,
        side=side,
...
def _add_room_label(
    ...
    def draw(draw, project) -> None:  # type: ignore[no-untyped-def]
        ...
        draw.text(
            (sx - width / 2, sy - 14), label, fill=(31, 34, 32, 220), font=font_label
        )
        if detail:
...
def _add_cabinet(
    ...
    if max(width, depth) > 0.9:
        ...
        for index in range(1, divisions):
            ...
            _draw_projected_polyline(
                renderer, line, y=0.81, fill=(75, 62, 49, 115), width=1
            )
    if obj.label:
...
def _opening_interval_on_wall(
    ...
    if (
        distance > tolerance
        or along < -endpoint_margin
        or along > length + endpoint_margin
    ):
        return None
...
def _split_wall_for_openings(
    ...
    for start, end in merged:
        if start - cursor >= min_segment:
            segments.append(
                (_point_along_wall(wall, cursor), _point_along_wall(wall, start))
            )
        cursor = max(cursor, end)
    if length - cursor >= min_segment:
        segments.append(
            (_point_along_wall(wall, cursor), _point_along_wall(wall, length))
        )
...
def _add_floor_texture_lines(
    ...
    def draw(draw, project) -> None:  # type: ignore[no-untyped-def]
        for wall, start_px, end_px in wall_segments:
            ...
            color = (
                (36, 38, 36, 54) if wall.wall_type == "exterior" else (60, 64, 60, 42)
            )
            draw.line(
...
def build_maricopa_source_aligned_scene() -> SemanticScene:
    ...
    walls = [
        ...
        SemanticWall(
            "w_garage_w", (50, 900), (50, 525), "exterior", "garage west wall"
        ),
        SemanticWall(
            "w_garage_s", (50, 900), (493, 900), "exterior", "garage south wall"
        ),
        SemanticWall(
            "w_garage_e", (493, 900), (493, 525), "interior", "garage east wall"
        ),
        SemanticWall(
            "w_garage_n", (50, 525), (493, 525), "interior", "garage north wall"
        ),
        SemanticWall("w_bed2_w", (50, 525), (50, 326), "exterior", "west bedroom wall"),
        SemanticWall(
            "w_bed2_n", (50, 326), (300, 326), "exterior", "west bedroom north wall"
        ),
        SemanticWall(
            "w_bed2_e", (300, 326), (300, 525), "interior", "west bedroom east wall"
        ),
        SemanticWall(
            "w_upperbed_w", (288, 286), (288, 76), "exterior", "upper bedroom west wall"
        ),
        SemanticWall(
            "w_upperbed_n", (288, 76), (498, 76), "exterior", "upper bedroom north wall"
        ),
        SemanticWall(
            "w_upperbed_e", (498, 76), (498, 330), "interior", "upper bedroom east wall"
        ),
        SemanticWall(
            "w_upperbed_s",
            (288, 286),
        ...
        SemanticWall(
            ...
            (950, 235),
            "exterior",
            "patio south wall with windows",
        ),
        SemanticWall("w_patio_e", (950, 25), (950, 235), "exterior", "patio east edge"),
        SemanticWall(
            "w_living_e", (950, 235), (950, 585), "interior", "living east wall"
        ),
        SemanticWall(
            "w_living_s", (675, 735), (858, 735), "interior", "dining south wall"
        ),
        SemanticWall(
            "w_kitchen_w",
            (493, 410),
        ...
        SemanticWall(
            ...
            (650, 505),
            "interior",
            "angled kitchen partition",
        ),
        SemanticWall(
            "w_laundry_s", (493, 760), (615, 760), "interior", "laundry south wall"
        ),
        # Right-side suite, foyer and office.
        SemanticWall(
            "w_master_n", (950, 27), (1182, 27), "exterior", "master bedroom north wall"
        ),
        SemanticWall(
            "w_master_e",
            (1182, 27),
        ...
        SemanticWall(
            "w_mbath_e", (1182, 294), (1182, 637), "exterior", "master bath east wall"
        ),
        SemanticWall(
            "w_mbath_s", (970, 637), (1182, 637), "interior", "master bath south wall"
        ),
        SemanticWall(
            "w_closet_w", (970, 388), (970, 637), "interior", "closet west wall"
        ),
        SemanticWall(
            "w_shower_diag",
            (1035, 590),
            (1088, 540),
            "interior",
            "closet/shower diagonal",
        ),
        SemanticWall(
            "w_foyer_w", (858, 585), (858, 735), "interior", "foyer west wall"
        ),
        SemanticWall(
            "w_foyer_e", (970, 585), (970, 735), "interior", "foyer east wall"
        ),
        SemanticWall(
            "w_entry_s", (858, 790), (970, 790), "exterior", "entry south wall"
        ),
        SemanticWall(
            "w_office_w", (970, 637), (970, 872), "interior", "office west wall"
        ),
        SemanticWall(
            "w_office_e", (1182, 637), (1182, 872), "exterior", "office east wall"
        ),
        SemanticWall(
            "w_office_s", (970, 872), (1182, 872), "exterior", "office south wall"
        ),
    ]
    ...
    openings = [
        ...
        SemanticOpening(
            "door_garage_w", "door", (50, 585), 82, 72, "", "garage exterior door"
        ),
        SemanticOpening(
            "door_bed2_hall", "door", (322, 360), 72, 34, "", "bedroom to hall"
        ),
        SemanticOpening(
        ...
        SemanticOpening(
            "door_patio_master", "door", (948, 247), 76, -45, "", "patio/living door"
        ),
        SemanticOpening(
            "door_master_bath", "door", (1118, 332), 85, 40, "", "master bath door"
        ),
        SemanticOpening(
            "door_foyer_entry", "door", (905, 680), 80, 0, "", "foyer door"
        ),
        SemanticOpening(
            "door_entry_out", "door", (860, 740), 78, 84, "", "entry exterior door"
        ),
        SemanticOpening(
            "door_laundry_garage",
            "door",
    ...
    objects = [
        ...
        SemanticObject(
            "kitchen_cab_1", "cabinet_run", (558, 470), 42, 168, 0, "cabinet"
        ),
        SemanticObject(
            "kitchen_cab_2", "cabinet_run", (638, 498), 45, 250, 0, "range/dw"
        ),
        ...
        SemanticObject(
            "laundry_washer", "washer_dryer", (625, 745), 44, 50, 0, "washer"
        ),
        ...
        SemanticObject(
            "closet_drawers", "cabinet_run", (1040, 512), 75, 34, -45, "drawers"
        ),
    ...
    labels = [
        SemanticRoomLabel(
            "BEDROOM", "12'-0\" x 11'-6\"", (180, 385), "BEDROOM 12'-0\" x 11'-6\""
        ),
        SemanticRoomLabel(
            "GARAGE", "24'-6\" x 23'-6\"", (290, 585), "GARAGE 24'-6\" x 23'-6\""
        ),
        SemanticRoomLabel(
            "BEDROOM", "12'-0\" x 12'-0\"", (395, 120), "BEDROOM 12'-0\" x 12'-0\""
        ),
        SemanticRoomLabel(
            "BATHROOM", "4'-6\" x 8'-0\"", (410, 455), "BATHROOM 4'-6\" x 8'-0\""
        ),
        SemanticRoomLabel(
            "BREAKFAST",
            "NOOK 6'-6\" x 6'-0\"",
            (565, 255),
            "BREAKFAST NOOK 6'-6\" x 6'-0\"",
        ),
        SemanticRoomLabel(
            "KITCHEN", "5'-6\" x 15'-6\"", (575, 455), "KITCHEN 5'-6\" x 15'-6\""
        ),
        SemanticRoomLabel(
            "LAUNDRY", "4'-0\" x 9'-6\"", (565, 705), "LAUNDRY 4'-0\" x 9'-6\""
        ),
        SemanticRoomLabel(
            "PATIO", "27'-6\" x 12'-0\"", (710, 85), "PATIO 27'-6\" x 12'-0\""
        ),
        SemanticRoomLabel(
            "LIVING ROOM",
            "16'-6\" x 21'-0\"",
            (800, 285),
            "LIVING ROOM 16'-6\" x 21'-0\"",
        ),
        SemanticRoomLabel(
            "DINING ROOM",
            "11'-0\" x 10'-0\"",
        ...
        SemanticRoomLabel(
            "FOYER", "6'-0\" x 6'-0\"", (900, 620), "FOYER 6'-0\" x 6'-0\""
        ),
        SemanticRoomLabel(
            "ENTRY", "7'-0\" x 5'-6\"", (905, 730), "ENTRY 7'-0\" x 5'-6\""
        ),
        SemanticRoomLabel(
            "MASTER BDRM",
            "13'-6\" x 15'-6\"",
            (1065, 75),
            "MASTER BDRM 13'-6\" x 15'-6\"",
        ),
        SemanticRoomLabel(
            "CLOSET", "5'-0\" x 9'-0\"", (1020, 435), "CLOSET 5'-0\" x 9'-0\""
        ),
        SemanticRoomLabel(
            "MASTER BATH",
            "5'-0\" x 16'-6\"",
            (1115, 410),
            "MASTER BATH 5'-0\" x 16'-6\"",
        ),
        SemanticRoomLabel(
            "OFFICE", "12'-6\" x 12'-0\"", (1075, 675), "OFFICE 12'-6\" x 12'-0\""
        ),
    ]

    tags = [
        SemanticIssueTag(
            "ISSUE-01", (190, 338), "red", "right", "egress window verification"
        ),
        SemanticIssueTag(
            "RFI-01", (540, 360), "blue", "right", "kitchen appliance clearance"
        ),
        SemanticIssueTag(
            "CLASH-01", (1040, 545), "yellow", "left", "shower/closet diagonal review"
        ),
        SemanticIssueTag(
            "ISSUE-02", (1168, 610), "red", "left", "tub access verification"
        ),
    ]
...
if __name__ == "__main__":
    print(
        json.dumps(
            build_maricopa_source_aligned_artifacts(
                Path("docs/plan2field3d_house_conversion")
            ),
            indent=2,
        )
    )

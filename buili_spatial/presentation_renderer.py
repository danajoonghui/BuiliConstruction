...
from .io_utils import atomic_save_image
...
class IsoRenderer:
    ...
    def add_box(
        ...
        *,
        ...
        p011 = (x0, y1, z1)
        self.add_surface(
            [p001, p101, p111, p011], top, outline=outline, layer=layer + 2
        )
    ...
    def render(self, path: Path) -> None:
        ...
        shadow = shadow.filter(
            ImageFilter.GaussianBlur(radius=int(18 * self.supersample))
        )
        image.paste(shadow, mask=shadow.getchannel("A"))

        draw = ImageDraw.Draw(image, "RGBA")
        ordered = sorted(
            self.surfaces, key=lambda surface: (surface.layer, self._depth(surface))
        )
        ...
        final = cropped.resize(
            (
                int(cropped.width / self.supersample),
                int(cropped.height / self.supersample),
            ),
            Image.Resampling.LANCZOS,
        )
        path.parent.mkdir(parents=True, exist_ok=True)
        atomic_save_image(path, final, compress_level=3)
...
def _add_floor_grid(
    renderer: IsoRenderer, *, x0: float, z0: float, x1: float, z1: float
) -> None:
    def draw(
        draw: ImageDraw.ImageDraw, project: Callable[[Point3D], tuple[float, float]]
    ) -> None:
        step = 1.0
...
def _add_label(
    renderer: IsoRenderer, label: str, room_no: str, x: float, z: float
) -> None:
    def draw(
        draw: ImageDraw.ImageDraw, project: Callable[[Point3D], tuple[float, float]]
    ) -> None:
        ...
        draw.text(
            (sx - width / 2, sy - 22), label, fill=(32, 34, 32, 225), font=font_label
        )
        if room_no:
...
def _add_tag(
    ...
    *,
    ...
    def draw(
        ...
        draw.text(
            (box[0] + padding, box[1] + 4), label, fill=(255, 255, 255, 255), font=font
        )
...
def _add_grid_bubble(renderer: IsoRenderer, label: str, point: Point3D) -> None:
    def draw(
        ...
        x, y = project(point)
        draw.ellipse(
            [x - 22, y - 22, x + 22, y + 22], outline=(55, 58, 55, 130), width=1
        )
        width = draw.textlength(label, font=font)
...
def render_reference_grade_demo(path: Path) -> dict[str, int | str]:
    ...
    for start, end in [
        ...
        renderer.add_wall(
            *start, *end, thickness=0.22, height=2.75, side=glass_side, cap=cap
        )
    for start, end in [
        ((4.6, 0), (4.6, 10.5)),
        ((8.4, 5.6), (8.4, 10.5)),
...
if __name__ == "__main__":
    output = Path(
        "docs/plan2field3d_house_conversion/reference_grade_lightweight_preview.png"
    )

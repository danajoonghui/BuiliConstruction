...
from .io_utils import InputLimitError, validate_input_file

if TYPE_CHECKING:  # Database integration is optional for the pure spatial package.
...
def _page_rgb(path: Path, page_no: int, zoom: float) -> np.ndarray:
    candidate = validate_input_file(
        ...
        max_bytes=int(os.environ.get("BUILI_MAX_PDF_BYTES", str(256 * 1024 * 1024))),
    )
    if page_no < 1:
        raise ValueError("page_no is one-based and must be >= 1")
    if not math.isfinite(float(zoom)) or not 0.25 <= float(zoom) <= 8.0:
        raise ValueError("zoom must be finite and between 0.25 and 8.0")
    with fitz.open(candidate) as pdf:
        if pdf.needs_pass:
            raise ValueError(
                "encrypted PDF must be decrypted before spatial extraction"
            )
        max_pages = int(os.environ.get("BUILI_MAX_PDF_PAGES", "500"))
        if len(pdf) > max_pages:
            raise InputLimitError(
                f"PDF has {len(pdf)} pages; configured limit is {max_pages}"
            )
        if page_no > len(pdf):
            raise ValueError(f"page_no {page_no} exceeds PDF page count {len(pdf)}")
        page = pdf[page_no - 1]
        estimated_pixels = int(page.rect.width * zoom) * int(page.rect.height * zoom)
        max_pixels = int(os.environ.get("BUILI_MAX_RENDER_PIXELS", "80000000"))
        if estimated_pixels > max_pixels:
            raise InputLimitError(
                f"render would allocate about {estimated_pixels} pixels; configured limit is {max_pixels}"
            )
...
def _dark_mask(rgb: np.ndarray, *, y_window: tuple[float, float]) -> np.ndarray:
    if cv2 is None:
        raise RuntimeError(
            "opencv-python-headless is required for raster floor-plan extraction"
        )
...
def _cluster_axis_values(
    values: list[float], tolerance_px: float
) -> dict[float, float]:
    if not values:
        return {}
...
def snap_and_merge_segments(
    ...
    *,
    ...
    return sorted(
        ...
        key=lambda item: (
            item.orientation,
            round(item.fixed_px, 6),
            round(item.start_px, 6),
            round(item.end_px, 6),
        ),
    )
    ...
    for segment in segments:
        ...
        for other in perpendicular:
            if segment.orientation == "h":
                crosses_axis = (
                    other.start_px - junction_tolerance_px
                    <= segment.fixed_px
                    <= other.end_px + junction_tolerance_px
                )
            ...
            if not crosses_axis:
                continue
            if (
                min(segment.start_px, segment.end_px) - junction_tolerance_px
                <= cut_value
                <= max(segment.start_px, segment.end_px) + junction_tolerance_px
            ):
...
def _segments_from_image(rgb: np.ndarray) -> tuple[list[AxisSegment], dict[str, Any]]:
    ...
    split = split_segments_at_junctions(
        merged,
        junction_tolerance_px=max(7.0, min(rgb.shape[:2]) * 0.005),
        min_length_px=min_length,
    )
    metadata = {
        ...
        "junction_split_axis_segments": len(split),
        "min_length_px": min_length,
        "y_window": [0.16, 0.9],
    }
...
def extract_pdf_vector_axis_segments(
    path: Path,
    *,
    page_no: int,
    zoom: float,
    ...
    """Extract true PDF line/rectangle primitives before falling back to raster CV."""

    candidate = validate_input_file(path, allowed_suffixes={".pdf"})
    if page_no < 1:
        raise ValueError("page_no is one-based and must be >= 1")
    raw: list[AxisSegment] = []
    drawing_count = 0
    with fitz.open(candidate) as pdf:
        if pdf.needs_pass:
            raise ValueError("encrypted PDF must be decrypted before vector extraction")
        if page_no > len(pdf):
            raise ValueError(f"page_no {page_no} exceeds PDF page count {len(pdf)}")
        page = pdf[page_no - 1]
        for drawing in page.get_drawings():
            drawing_count += 1
            thickness = max(1.0, float(drawing.get("width") or 1.0) * zoom)
            for item in drawing.get("items") or []:
                command = item[0]
                pairs: list[tuple[fitz.Point, fitz.Point]] = []
                ...
                for start, end in pairs:
                    x0, y0 = float(start.x) * zoom, float(start.y) * zoom
                    x1, y1 = float(end.x) * zoom, float(end.y) * zoom
                    dx, dy = abs(x1 - x0), abs(y1 - y0)
                    axis_tolerance = max(1.5, min(dx + dy, 500.0) * 0.006)
                    if dy <= axis_tolerance and dx >= min_length_px:
                        raw.append(
                            AxisSegment(
                                "h", (y0 + y1) / 2, min(x0, x1), max(x0, x1), thickness
                            )
                        )
                    elif dx <= axis_tolerance and dy >= min_length_px:
                        raw.append(
                            AxisSegment(
                                "v", (x0 + x1) / 2, min(y0, y1), max(y0, y1), thickness
                            )
                        )
    merged = snap_and_merge_segments(
        raw,
        snap_tolerance_px=max(4.0, min_length_px * 0.18),
        merge_gap_px=max(7.0, min_length_px * 0.28),
        min_length_px=min_length_px,
    )
    split = split_segments_at_junctions(
        merged,
        junction_tolerance_px=max(5.0, min_length_px * 0.16),
        min_length_px=min_length_px,
    )
    return split, {
        "available": bool(raw),
        "drawing_count": drawing_count,
        "raw_axis_segments": len(raw),
        "merged_axis_segments": len(merged),
        "junction_split_axis_segments": len(split),
        "method": "pymupdf_vector_primitives_snap_merge",
    }

...
from .floorplan_extractor import (
    ...
    extract_pdf_vector_axis_segments,
)
from .io_utils import atomic_save_image, atomic_write_json
    ...
    for segment in segments:
        ...
        if segment.orientation == "h":
            ...
            if length > image_width * 0.72 and segment.fixed_px > image_height * 0.74:
                continue
            if (
                segment.fixed_px < image_height * 0.12
                or segment.fixed_px > image_height * 0.9
            ):
                continue
        else:
            ...
            if length > image_height * 0.72 and segment.fixed_px < image_width * 0.12:
                continue
            if (
                segment.fixed_px < image_width * 0.02
                or segment.fixed_px > image_width * 0.98
            ):
                continue
...
def _segments_touch(
    left: AxisSegment, right: AxisSegment, *, tolerance_px: float
) -> bool:
    """Return whether two axis-aligned primitives belong to one line network.

    PDF drawing APIs usually split walls at joins, doors, and changes in line
    style.  Treating near joins as connected lets us separate the dominant plan
    network from page borders, title blocks, legends, and note underlines.
    """

    left_start, left_end = sorted((left.start_px, left.end_px))
    right_start, right_end = sorted((right.start_px, right.end_px))
    if left.orientation == right.orientation:
        if abs(left.fixed_px - right.fixed_px) > tolerance_px:
            return False
        return max(left_start, right_start) <= min(left_end, right_end) + tolerance_px
    horizontal = left if left.orientation == "h" else right
    vertical = right if left.orientation == "h" else left
    horizontal_start, horizontal_end = sorted(
        (horizontal.start_px, horizontal.end_px)
    )
    vertical_start, vertical_end = sorted((vertical.start_px, vertical.end_px))
    return (
        horizontal_start - tolerance_px
        <= vertical.fixed_px
        <= horizontal_end + tolerance_px
        and vertical_start - tolerance_px
        <= horizontal.fixed_px
        <= vertical_end + tolerance_px
    )


def _tight_segment_bounds(
    segments: list[AxisSegment], image_width: int, image_height: int
) -> tuple[int, int, int, int]:
    if not segments:
        return 0, 0, image_width, image_height
    ...
    return (
        ...
        min(image_width, int(math.ceil(max(xs)))),
        min(image_height, int(math.ceil(max(ys)))),
    )


def _dominant_plan_segments(
    segments: list[AxisSegment], image_width: int, image_height: int
) -> tuple[list[AxisSegment], dict[str, Any]]:
    """Keep the dominant, plan-like line network and its interior primitives.

    The winning component must contain both horizontal and vertical geometry.
    Score combines occupied area, line length, balance of orientations, and
    junctions.  Additional enclosed components are retained only when they are
    themselves two-axis line networks, which excludes annotation boxes and text
    baselines while preserving detached cores or plan insets.
    """
    ...
    tolerance_px = max(8.0, min(image_width, image_height) * 0.006)
    ...
    for left_index, left in enumerate(segments):
        for right_index in range(left_index + 1, len(segments)):
            if _segments_touch(
                left, segments[right_index], tolerance_px=tolerance_px
            ):
    ...
    page_area = max(float(image_width * image_height), 1.0)
    scored: list[dict[str, Any]] = []
    for indices in components.values():
        component = [segments[index] for index in indices]
        bbox = _tight_segment_bounds(component, image_width, image_height)
        width = max(float(bbox[2] - bbox[0]), 1.0)
        height = max(float(bbox[3] - bbox[1]), 1.0)
        area_ratio = width * height / page_area
        horizontal_count = sum(item.orientation == "h" for item in component)
        vertical_count = len(component) - horizontal_count
        orientation_balance = (
            min(horizontal_count, vertical_count)
            / max(horizontal_count, vertical_count)
            if horizontal_count and vertical_count
            else 0.0
        )
        total_length = sum(_segment_length(item) for item in component)
        junctions = 0
        for left_index, left in enumerate(component):
            for right in component[left_index + 1 :]:
                if left.orientation != right.orientation and _segments_touch(
                    left, right, tolerance_px=tolerance_px
                ):
                    junctions += 1
        aspect_ratio = max(width, height) / max(min(width, height), 1.0)
        aspect_penalty = 0.3 if aspect_ratio > 12.0 else 1.0
        score = (
            total_length
            * math.sqrt(max(area_ratio, 1e-6))
            * (0.2 + 0.8 * orientation_balance)
            * (1.0 + min(junctions, 20) * 0.08)
            * aspect_penalty
        )
        scored.append(
            {
    ...
    viable = [
        item
        for item in scored
        if item["horizontal_count"] > 0
        and item["vertical_count"] > 0
        and item["area_ratio"] >= 0.01
    ]
    if not viable:
        return segments, {
            "applied": False,
            "reason": "no_plan_like_component",
            "component_count": len(scored),
        }

    winner = max(viable, key=lambda item: float(item["score"]))
    winner_component = [segments[index] for index in winner["indices"]]
    crop_bbox = _axis_segment_bounds(winner_component, image_width, image_height)
    selected_indices = set(int(index) for index in winner["indices"])
    for component in scored:
        if component is winner:
            continue
        if (
            component["horizontal_count"] <= 0
            or component["vertical_count"] <= 0
            or component["junctions"] <= 0
            or float(component["total_length_px"])
            < float(winner["total_length_px"]) * 0.04
        ):
            continue
        component_bbox = tuple(int(value) for value in component["bbox"])
        if not (
            crop_bbox[0] <= component_bbox[0]
            and crop_bbox[1] <= component_bbox[1]
            and component_bbox[2] <= crop_bbox[2]
            and component_bbox[3] <= crop_bbox[3]
        ):
            continue
        selected_indices.update(int(index) for index in component["indices"])
    selected = [
        segment for index, segment in enumerate(segments) if index in selected_indices
    ]
    runner_up = sorted(
        (float(item["score"]) for item in viable if item is not winner),
        reverse=True,
    )
    score_margin = (
        float(winner["score"]) / max(runner_up[0], 1e-6)
        if runner_up
        else float("inf")
    )
    return selected, {
        "applied": True,
        "reason": "dominant_plan_network",
        "component_count": len(scored),
        "selected_segments": len(selected),
        "dominant_component_segments": len(winner_component),
        "dominant_bbox_px": list(winner["bbox"]),
        "dominant_score": round(float(winner["score"]), 4),
        "score_margin": (
            round(score_margin, 4) if math.isfinite(score_margin) else None
        ),
        "area_ratio": round(float(winner["area_ratio"]), 6),
        "junctions": int(winner["junctions"]),
        "tolerance_px": round(tolerance_px, 3),
    }
...
def _crop_segments(
    ...
    for segment in segments:
        ...
        if end - start >= 16:
            cropped.append(
                AxisSegment(
                    segment.orientation, fixed, start, end, segment.thickness_px
                )
            )
    return cropped
...
def _easyocr_reader() -> Any | None:
    ...
    if easyocr is None:
        return None
    if _EASYOCR_READER_ERROR:
        return None
    if _EASYOCR_READER is None:
        gpu_enabled = os.environ.get("BUILI_EASYOCR_GPU", "").lower() in {
            "1",
            "true",
            "yes",
        }
...
def _easyocr_text_items_from_input(
    ...
    if reader is None:
        return [], {
            "engine": "none",
            "available": False,
            "reason": _EASYOCR_READER_ERROR or "easyocr_not_installed",
        }
...
def _is_room_label(text: str) -> str | None:
    key = _upper_key(text)
    dimension_match = DIMENSION_RE.search(key)
    if dimension_match:
        key = f"{key[: dimension_match.start()]} {key[dimension_match.end() :]}"
    key = re.sub(r"\s+", " ", key).strip(" .-")
    # Room tags are compact labels, not sentences which happen to mention a room.
    # Allow one trailing room number, as used by many architectural sheets.
    key = re.sub(r"\s+#?[A-Z]?\d{1,4}[A-Z]?$", "", key).strip()
    if not key or len(key) > 32 or len(key.split()) > 3:
        return None
...
def _point_in_face_bounds(
    ...
    *,
    ...
    x, y = point
    return any(
        x0 + margin_px <= x <= x1 - margin_px
        and y0 + margin_px <= y <= y1 - margin_px
        for x0, y0, x1, y1 in face_bounds
    )


def _point_has_room_enclosure(
    point: tuple[float, float], segments: list[AxisSegment]
) -> bool:
    """Require wall evidence around a room tag, not just a page-level bbox."""
        ...
        max(
            [
                ...
                *(
                    segment.fixed_px
                    if segment.orientation == "h"
                    else max(segment.start_px, segment.end_px)
                    for segment in segments
                ),
            ]
        )
    ...
    min_x, min_y, max_x, max_y = _tight_segment_bounds(
        segments,
        max(1, image_width),
        max(1, image_height),
    )
    tolerance_px = max(10.0, min(max_x - min_x, max_y - min_y) * 0.025)
    left = right = above = below = False
    for segment in segments:
        start, end = sorted((segment.start_px, segment.end_px))
        if segment.orientation == "v" and start - tolerance_px <= y <= end + tolerance_px:
            left = left or segment.fixed_px < x
            right = right or segment.fixed_px > x
        elif (
            segment.orientation == "h"
            and start - tolerance_px <= x <= end + tolerance_px
        ):
            above = above or segment.fixed_px < y
            below = below or segment.fixed_px > y
    side_count = sum((left, right, above, below))
    return side_count >= 3 and (left or right) and (above or below)


def _room_labels_from_text(
    text_items: list[TextItem],
    *,
    face_bounds: list[tuple[int, int, int, int]] | None = None,
    face_segments: list[AxisSegment] | None = None,
) -> list[SemanticRoomLabel]:
    labels: list[SemanticRoomLabel] = []
    for item in text_items:
        room_name = _is_room_label(item.text)
        if not room_name:
            continue
        if face_bounds is not None and not _point_in_face_bounds(
            item.center, face_bounds, margin_px=2.0
        ):
            continue
        if face_segments is not None and not _point_has_room_enclosure(
            item.center, face_segments
        ):
            continue
...
def _windows_from_colored_markers(
    ...
    for index in range(1, count):
        ...
        for offset, has_pixel in enumerate(col_has):
            if has_pixel:
                ...
                start is not None and last_seen is not None and offset - last_seen > 18
            ):
    ...
    for index, group in enumerate(grouped, start=1):
        ...
        angle = (
            0.0
            if abs(wall.end_px[0] - wall.start_px[0])
            >= abs(wall.end_px[1] - wall.start_px[1])
            else 90.0
        )
...
_FIXTURE_TEXT_TAGS: dict[str, tuple[str, str]] = {
    "W": ("washer_dryer", "washer"),
    "WASH": ("washer_dryer", "washer"),
    "WASHER": ("washer_dryer", "washer"),
    "D": ("washer_dryer", "dryer"),
    "DRY": ("washer_dryer", "dryer"),
    "DRYER": ("washer_dryer", "dryer"),
    "W D": ("washer_dryer", "washer_dryer"),
    "WH": ("water_heater", "wh"),
    "W H": ("water_heater", "wh"),
    "WATER HEATER": ("water_heater", "wh"),
    "SINK": ("sink", "sink"),
    "LAV": ("sink", "lav"),
    "LAVATORY": ("sink", "lav"),
    "RANGE": ("cabinet_run", "range"),
    "DW": ("cabinet_run", "dw"),
    "D W": ("cabinet_run", "dw"),
    "REF": ("cabinet_run", "ref"),
    "REFRIG": ("cabinet_run", "refrig"),
    "REFRIGERATOR": ("cabinet_run", "refrigerator"),
    "FREEZER": ("cabinet_run", "freezer"),
    "SD": ("smoke_detector", "sd"),
    "SMOKE DET": ("smoke_detector", "sd"),
    "SMOKE DETECTOR": ("smoke_detector", "sd"),
    "CL": ("ceiling_light", "light"),
    "CEILING LIGHT": ("ceiling_light", "light"),
    "REC": ("duplex_outlet", "rec"),
    "RECEPT": ("duplex_outlet", "rec"),
    "RECEPTACLE": ("duplex_outlet", "rec"),
    "OUTLET": ("duplex_outlet", "rec"),
    "DUPLEX": ("duplex_outlet", "rec"),
    "GFI": ("duplex_outlet", "gfi"),
    "GFCI": ("duplex_outlet", "gfci"),
    "GFI REC": ("duplex_outlet", "gfi"),
    "GFCI REC": ("duplex_outlet", "gfci"),
}


def _fixture_from_compact_text(text: str) -> tuple[str, str] | None:
    key = _upper_key(text)
    # Prose, leaders, and note references are evidence text, not fixture symbols.
    if (
        not key
        or len(key) > 24
        or len(key.split()) > 3
        ...
        or "NOTE" in key
        or DIMENSION_RE.search(key)
    ):
        return None
    mapped = _FIXTURE_TEXT_TAGS.get(key)
    if mapped:
        return mapped
    if re.fullmatch(r"(?:S|SW|S[234]|3W|4W)", key):
        return "switch", "sw"
    return None


def _objects_from_text(
    text_items: list[TextItem],
    *,
    face_bounds: list[tuple[int, int, int, int]] | None = None,
) -> list[SemanticObject]:
    ...
    index = 1
    for item in text_items:
        fixture = _fixture_from_compact_text(item.text)
        if fixture is None:
            continue
        if face_bounds is not None and not _point_in_face_bounds(
            item.center, face_bounds, margin_px=0.0
        ):
            continue
        kind, label = fixture
...
def _dedupe_objects(
    ...
    laundry_centers = [
        label.center_px for label in labels if label.name in {"LAUNDRY", "UTILITY"}
    ]

    def nearest_distance(
        obj: SemanticObject, centers: list[tuple[float, float]]
    ) -> float:
        if not centers:
            return float("inf")
        ox, oy = obj.center_px
    ...
    def rank(obj: SemanticObject) -> tuple[int, float, float]:
        if obj.source_note.startswith(
            "micro_vlm_verified"
        ) or obj.source_note.startswith("micro_vlm_corrected"):
            source_rank = 0
...
def _dimensions_from_text(
    ...
    for item in text_items:
        ...
        dimensions.append(
            SemanticDimension((x0 - pad, y1 + 8), (x1 + pad, y1 + 8), dim)
        )
...
def _sample_line_points(
    ...
    for index in range(count):
        ...
        points.append(
            (int(round(sx + (ex - sx) * ratio)), int(round(sy + (ey - sy) * ratio)))
        )
    return points


def _distance_samples(
    distance: np.ndarray, points: list[tuple[int, int]]
) -> list[float]:
...
def evaluate_scene_alignment(
    ...
    for wall in scene.walls:
        values = _distance_samples(
            distance, _sample_line_points(wall.start_px, wall.end_px)
        )
    ...
    for opening in scene.openings:
        ...
        region = dark[y0:y1, x0:x1]
        dark_fraction = (
            float(np.count_nonzero(region) / max(region.size, 1)) if region.size else 0
        )
        opening_rows.append(
            {
                "id": opening.id,
                "kind": opening.kind,
                "center_px": [
                    round(opening.center_px[0], 2),
                    round(opening.center_px[1], 2),
                ],
    ...
    if overlay_png is not None:
        _write_alignment_overlay(
            scene, crop_png, overlay_png, weak_objects, wall_failures
        )
...
def _write_alignment_overlay(
    ...
    for opening in scene.openings:
        ...
        draw.ellipse(
            [cx - radius, cy - radius, cx + radius, cy + radius], outline=color, width=3
        )
    for obj in scene.objects:
    ...
    atomic_save_image(overlay_png, Image.alpha_composite(image, overlay).convert("RGB"))
...
def build_semantic_scene_from_pdf(
    ...
    *,
    ...
    zoom: float = 2.5,
    use_ocr: bool = True,
    use_micro_vlm: bool = True,
) -> tuple[SemanticScene, dict[str, Any]]:
    start = time.perf_counter()
    output_dir.mkdir(parents=True, exist_ok=True)
    full_page_png = output_dir / f"auto_page_{page_no}.png"
    crop_png = output_dir / "auto_plan_crop.png"

    stage_start = time.perf_counter()
    rgb = _page_rgb(pdf_path, page_no=page_no, zoom=zoom)
    atomic_save_image(full_page_png, Image.fromarray(rgb))
    page_render_seconds = time.perf_counter() - stage_start

    stage_start = time.perf_counter()
    raster_segments, wall_metadata = _segments_from_image(rgb)
    raster_floorplan_segments = _filter_floorplan_segments(
        raster_segments, rgb.shape[1], rgb.shape[0]
    )
    raster_floorplan_segments, raster_roi_metadata = _dominant_plan_segments(
        raster_floorplan_segments, rgb.shape[1], rgb.shape[0]
    )
    try:
        vector_segments, vector_metadata = extract_pdf_vector_axis_segments(
            pdf_path,
            page_no=page_no,
            zoom=zoom,
            min_length_px=float(wall_metadata.get("min_length_px") or 40.0),
        )
        vector_floorplan_segments = _filter_floorplan_segments(
            vector_segments, rgb.shape[1], rgb.shape[0]
        )
        vector_is_bounded = len(vector_floorplan_segments) <= max(
            2_000, len(raster_floorplan_segments) * 3
        )
        if vector_is_bounded:
            vector_floorplan_segments, vector_roi_metadata = (
                _dominant_plan_segments(
                    vector_floorplan_segments, rgb.shape[1], rgb.shape[0]
                )
            )
        else:
            vector_floorplan_segments = []
            vector_roi_metadata = {
                "applied": False,
                "reason": "vector_segment_count_unbounded",
                "segment_count": len(vector_segments),
            }
    except Exception as exc:
        vector_floorplan_segments = []
        vector_metadata = {"available": False, "reason": str(exc)}
        vector_roi_metadata = {
            "applied": False,
            "reason": "vector_extraction_failed",
        }
    if len(vector_floorplan_segments) >= 6:
        floorplan_segments = vector_floorplan_segments
        selected_geometry_source = "pdf_vector_primitives"
    else:
        floorplan_segments = raster_floorplan_segments
        selected_geometry_source = "raster_cv"
    wall_metadata["vector"] = vector_metadata
    wall_metadata["raster_plan_roi"] = raster_roi_metadata
    wall_metadata["vector_plan_roi"] = vector_roi_metadata
    wall_metadata["selected_geometry_source"] = selected_geometry_source
    ...
    atomic_save_image(crop_png, Image.fromarray(cropped_rgb))
    ...
    stage_start = time.perf_counter()
    text_items = _embedded_pdf_text_items(
        pdf_path, page_no=page_no, zoom=zoom, crop_bbox=crop_bbox
    )
    ...
    face_bounds = (
        [_tight_segment_bounds(cropped_segments, crop_width, crop_height)]
        if cropped_segments
        else []
    )
    labels = _room_labels_from_text(
        ...
        face_segments=cropped_segments,
    )
    text_objects = _objects_from_text(text_items, face_bounds=face_bounds)
    ...
    if use_micro_vlm:
        micro_vlm_objects, micro_vlm_openings, micro_vlm_metadata = (
            detect_micro_vlm_plan_elements(
                crop_png,
                labels=labels,
        ...
        micro_vlm_objects, micro_vlm_openings, micro_vlm_metadata = (
            [],
            [],
            {"enabled": False, "reason": "disabled_by_caller"},
        )
    ...
    metadata = {
        ...
        "detected_plan_face_bounds_px": [list(item) for item in face_bounds],
...
def _point_m(
    ...
    *,
    ...
    if meters_per_px is None:
        ...
        x = (point_px[0] - scene.transform.offset_x_px) * meters_per_px
        z = (
            scene.transform.height_px - point_px[1] + scene.transform.offset_y_px
        ) * meters_per_px
...
def semantic_scene_to_plan_graph_payload(
    ...
    *,
    ...
    px_per_meter = float(scale.get("px_per_meter") or 0.0)
    if not math.isfinite(px_per_meter) or px_per_meter <= 0:
        raise ValueError("scale.px_per_meter must be a positive finite number")
    meters_per_px = 1.0 / px_per_meter
    floor_polygon = [
        _point_m(scene, (x0, y1), meters_per_px=meters_per_px),
        _point_m(scene, (x1, y1), meters_per_px=meters_per_px),
        _point_m(scene, (x1, y0), meters_per_px=meters_per_px),
        _point_m(scene, (x0, y0), meters_per_px=meters_per_px),
    ]
    ...
    walls = [
        {
            "id": wall.id,
            "room_id": room_id,
            "from": _point_m(scene, wall.start_px, meters_per_px=meters_per_px),
            "to": _point_m(scene, wall.end_px, meters_per_px=meters_per_px),
    ...
    openings = [
        {
            ...
            "center_m": _point_m(scene, opening.center_px, meters_per_px=meters_per_px),
            "width_m": round(opening.length_px * meters_per_px, 3),
    ...
    fixtures = [
        {
            ...
            "center_m": _point_m(scene, obj.center_px, meters_per_px=meters_per_px),
    ...
    for label in scene.labels:
        label_point = _point_m(scene, label.center_px, meters_per_px=meters_per_px)
        sources.append(
            {
                "citation_chunk_id": "",
                "doc_id": source_doc_id,
    ...
    return {
        ...
        "extraction": {
            ...
            "meters_per_px": round(meters_per_px, 9),
            "scale_contract": "plan_graph_scale_applied_at_semantic_conversion",
        },
    }
...
def build_auto_plan2field3d_artifacts(
    ...
    *,
    ...
    atomic_write_json(scene_json, scene.to_json())
    ...
    summary = {
        ...
        "total_seconds": round(
            scene_metadata["total_scene_build_seconds"] + render_seconds, 4
        ),
        "qa": {
            "manual_semantic_seed_used": False,

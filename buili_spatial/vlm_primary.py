...
def _load_model(path: Path) -> tuple[Any | None, dict[str, Any]]:
    ...
    try:
        checkpoint = safe_torch_checkpoint_load(
            torch,
            path,
            expected_sha256_env="BUILI_PLAN2FIELD_VLM_PRIMARY_SHA256",
        )
    ...
    try:
        ...
        return None, {
            "enabled": False,
            "reason": "model_artifact_or_encoder_incompatible",
            "error": str(exc),
            "path": str(path),
        }
    device = (
...
def _nms(
    predictions: list[PlanTokenPrediction], iou_threshold: float = 0.22
) -> list[PlanTokenPrediction]:
...
def _prediction_to_object(
    pred: PlanTokenPrediction, index: int
) -> SemanticObject | None:
...
def generate_vlm_primary_plan_tokens(
    ...
    *,
    ...
    if os.environ.get("BUILI_PLAN2FIELD_VLM_PRIMARY_DISABLE", "").lower() in {
        "1",
        "true",
        "yes",
    }:
    ...
    proposal_records = [
        (
            _centered_patch_box(
                (cx, cy), width=image.width, height=image.height, patch=patch
            ),
            _source,
        )
        for cx, cy, _source in proposal_centers or []
    ]
    dense_enabled = os.environ.get(
        "BUILI_PLAN2FIELD_VLM_PRIMARY_DENSE", ""
    ).lower() in {
        "1",
        "true",
        "yes",
    ...
    dense_records = (
        [(box, "dense_grid") for box in dense_boxes]
        if dense_enabled or not proposal_records
        else []
    )
    ...
    for batch_start in range(0, len(box_records), batch_size):
        ...
        for _box, source in batch_records:
            if source.startswith("geometry_candidate_opening"):
                ...
                batch_task_specs.append(
                    [
                        (
                            VLM_PRIMARY_TASKS["detect_mep_fixtures"],
                            OBJECT_CLASSES,
                            "mep_fixture",
                        )
                    ]
                ...
                batch_task_specs.append(
                    [
                        (
                            VLM_PRIMARY_TASKS["detect_openings"],
                            OPENING_CLASSES,
                            "opening",
                        ),
                        (
        ...
        task_ids_to_run = sorted(
            {spec[0] for specs in batch_task_specs for spec in specs}
        )
        for task_id in task_ids_to_run:
            ...
            for row, patch_box in enumerate(batch_boxes):
                matching_specs = [
                    spec for spec in batch_task_specs[row] if spec[0] == task_id
                ]
